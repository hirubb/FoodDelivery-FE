...
const AddressBook = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // If this is marked as default and there are existing addresses,
      // we need to update the current default to non-default
      if (formData.isDefault && addresses.some(addr => addr.isDefault)) {
        await UserService.updateDefaultAddress(userId, addressData);
      } else {
        // Otherwise, simply add the new address
        await UserService.addUserAddress(userId, addressData);
      }
            ...
            <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
              ...
              <div className="flex justify-end gap-3 md:col-span-2">
                ...
                <button
                  ...
                  className="px-4 py-2 text-white transition-colors bg-orange-500 rounded-md hover:bg-orange-600"
                >
                  {formData.addressId ? 'Update Address' : 'Save Address'}
                </button>
              </div>
            </div>
          </form>
